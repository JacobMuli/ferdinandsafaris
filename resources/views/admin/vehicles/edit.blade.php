...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800 ">Edit Vehicle Unit</h2>
            ...
            <form action="{{ route('admin.vehicles.update', $vehicle) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="year" name="year" type="number" min="1900" max="{{ date('Y') + 1 }}" class="mt-1 block w-full" :value="old('year', $vehicle->year)" required />
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full border-gray-300    rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @foreach(['available', 'in_use', 'maintenance', 'retired'] as $s)
                                <option value="{{ $s }}" {{ old('status', $vehicle->status) === $s ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <x-text-input id="mileage" name="mileage" type="number" min="0" class="mt-1 block w-full" :value="old('mileage', $vehicle->mileage)" />
                        <x-input-error :messages="$errors->get('mileage')" class="mt-2" />
                    </div>
                </div>

                <div class="flex items-center space-x-2">
                    <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500" {{ old('is_active', $vehicle->is_active) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300    rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $vehicle->notes) }}</textarea>
